...
class Program {
    static bool _isListening, _elif;
                ...
                using (var waveIn = new WaveInEvent()) {
                    ...
                    waveIn.DataAvailable += (sender, e) => {
                        ...
                        if (_isListening && recognizer.AcceptWaveform(e.Buffer, e.BytesRecorded)) {
                            ...
                            _isListening = false; // Ждемо некст ключ
                            _elif = false;
                        }
                    };

                    waveIn.StartRecording();
                    Console.WriteLine("Голосовой ассистент 'Астра' запущен.");
                    ProcessCom.StartHi();
                    Console.ReadLine();
    ...
    static void ProcessCommand(string text) {
        ...
        else if (text.Contains("включи музыку")) {
            ProcessCom.ConfS();
        ...
        */
                 ...
                 text.Contains("снимок") || text.Contains("сделай скрин")) {
            Process.Start("D:\\Projects\\Pulse\\astraCommands\\windows\\ahk\\screenshot.exe");
            ProcessCom.ConfS();
        }
